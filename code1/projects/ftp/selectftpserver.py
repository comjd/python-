...
import queue
from projects.ftp.fileprocess import FileProcess as fp
from projects.ftp.infotype import InfoType as info

FTP_URL = '/home/tyler/dev/code1/projects/ftp/test_tcp_files/'
...
class FTPServer:
    ...
    def get_file_name_list(self):
        result = ''
        for file in fp.get_path_all_filenames(FTP_URL):
            result += file + '\n'
        self.connfd.send(result.encode())

    def download_file(self, filename):
        if fp.file_exists(FTP_URL + filename):
            self.connfd.send(info.PACKAGE_START.value.encode())
            for line in fp.read_line_file(FTP_URL + filename):
                self.connfd.send(line)
            else:
                self.connfd.send(info.PACKAGE_END.value.encode())
        else:
            self.connfd.send(('文件不能存在' + info.PACKAGE_END.value).encode())
    ...
    while input:
        ...
        for s in writable:
            try:
                ...
                # 发送文件列表
                if next_msg.decode().split(' ')[0] == info.LIST.value.lower():
                    fs.get_file_name_list()
                elif next_msg.decode().split(' ')[0] == info.DOWN.value.lower():
                    filename = next_msg.decode().split(' ')[1]
                    fs.download_file(filename)
                    # if fp.file_exists(FTP_URL + filename):
                    #     s.send(info.PACKAGE_START.value.encode())
                    #     for line in fp.read_line_file(FTP_URL + filename):
                    #         s.send(line)
                    #     else:
                    #         s.send(info.PACKAGE_END.value.encode())
                    # else:
                    #     s.send('文件不能存在'.encode())
                else:
                    s.send(info.PACKAGE_END.value.encode())
